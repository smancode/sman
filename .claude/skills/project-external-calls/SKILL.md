...
| **SQLite (better-sqlite3)** | 嵌入式数据库 | 会话/消息/Cron/Batch/Chatbot/Stardom/Hub/Group 持久化 | [references/sqlite.md](references/sqlite.md) |
...
- **🆕 Group**: 跨工作区协作（3 表：groups/group_tasks/group_subtasks）+ `~/.sman/group/{groupId}/` 目录
- **🔄 Git**: 字符串→数组参数（防注入，`['status', '--porcelain']`）
- **🆕 Hub Feedback**: `POST /api/feedback` + `/api/error-report`（PSK 加密，降级到 `~/logs/`）
- **🔄 Chatbot**: 空闲超时（15min）+ `chat_type` 字段 + `clearSdkSessionId()`
- **🔄 SmartPath**: AI 指南生成（`smartpath.guideChat/guideSave`）
