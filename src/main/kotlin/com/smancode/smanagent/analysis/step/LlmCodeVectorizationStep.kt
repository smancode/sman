...
import com.smancode.smanagent.analysis.coordination.CodeVectorizationCoordinator
...
class LlmCodeVectorizationStep(
    ...
    private val llmService: com.smancode.smanagent.smancode.llm.LlmService,
    ...
    override suspend fun canExecute(context: AnalysisContext): Boolean {
        // 检查是否启用了 LLM 向量化
        if (!SmanAgentConfig.analysisLlmVectorizationEnabled) {
    ...
    override suspend fun execute(context: AnalysisContext): StepResult = withContext(Dispatchers.IO) {
        ...
        try {
            logger.info("开始 LLM 驱动的代码向量化: projectKey={}, path={}, fullRefresh={}",
                projectKey, projectPath, SmanAgentConfig.analysisLlmVectorizationFullRefresh)
            ...
            // 执行向量化（使用配置决定是否全量刷新）
            val forceUpdate = SmanAgentConfig.analysisLlmVectorizationFullRefresh
