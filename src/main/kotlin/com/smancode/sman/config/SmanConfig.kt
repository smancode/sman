...
object SmanConfig {
    ...
    private val logger = LoggerFactory.getLogger(SmanConfig::class.java)
    ...
    val llmApiKey: String
        get() = userConfig?.llmApiKey?.takeIf { it.isNotEmpty() }
            ...
            ?: throw IllegalStateException(
                ...
                |
                |2. 配置文件: src/main/resources/sman.properties
    ...
    val bgeM3Config: BgeM3Config? by lazy {
        ...
        if (endpoint.isBlank()) {
            ...
            val truncationStrategy = try {
                com.smancode.sman.analysis.config.TruncationStrategy.valueOf(truncationStrategyStr.uppercase())
            } catch (e: IllegalArgumentException) {
                logger.warn("未知的截断策略: $truncationStrategyStr，使用默认值 TAIL")
                com.smancode.sman.analysis.config.TruncationStrategy.TAIL
            }
    ...
    private fun loadProperties(): Properties {
        ...
        try {
            // 从 classpath 加载配置文件
            inputStream = javaClass.getResourceAsStream("/sman.properties")
            if (inputStream != null) {
                props.load(inputStream)
                logger.info("成功加载配置文件: sman.properties")
            } else {
                logger.warn("配置文件 sman.properties 未找到，将使用默认配置和环境变量")
            }
        } catch (e: Exception) {
