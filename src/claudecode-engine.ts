import type { EngineHealth, ExecutionRequest, ExecutionResult } from "./types.js";
import type { ExecutionEngine } from "./registry.js";
import type { AcpxClient } from "./acpx.js";

export type ClaudeCodeRunner = (prompt: string, taskId: string) => Promise<string>;
...
export const createAcpxClaudeCodeRunner = (client: AcpxClient): ClaudeCodeRunner => {
...
const defaultRunner: ClaudeCodeRunner = async (prompt, taskId) => {
  ...
  throw new Error("SMAN_CLAUDECODE_STUB_OUTPUT 未配置，当前仅支持注入 ACPX runner 或设置 stub 输出");
};
