      ...
      if (response.success && response.data) {
        ...
        update((state) => {
          ...
          const newActiveTaskId =
            taskData.status !== "running" && state.activeTaskId === taskId
              ? null
              : state.activeTaskId;
          return {
            ...state,
        ...
        update((state) => {
          ...
          const tasks = state.tasks.map((t) => {
            ...
            const mappedAction = (action.charAt(0).toUpperCase() +
              action.slice(1)) as "Created" | "Modified" | "Deleted";
      ...
      const unlistenSubTaskStarted = await onSubTaskStarted(
        (event: SubTaskStartedEvent) => {
          update((state) => {
            ...
            const exists = current.some((task) => task.id === event.subtask_id);
            const next = exists
              ? current.map((task) =>
      ...
      const unlistenSubTaskCompleted = await onSubTaskCompleted(
        (event: SubTaskCompletedEvent) => {
          update((state) => {
            ...
            if (!next.some((task) => task.id === event.subtask_id)) {
              next.push({
                id: event.subtask_id,
            ...
            tasks: state.tasks.map((task) =>
              task.id === event.task_id
                ? {
                    ...task,
                    output: event.output,
                    error: event.passed ? task.error : event.output,
                  }
                : task,
            ),
          }));
