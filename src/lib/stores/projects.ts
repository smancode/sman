  ...
  return {
    ...
    async loadProjects() {
      ...
      if (response.success && response.data) {
        update((state) => {
          const hiddenProjectIds = state.hiddenProjectIds.filter((id) =>
    ...
    reorderProjectsByOrder(nextOrder: string[]) {
      ...
      update((state) => {
        ...
        const normalized = normalizeProjectOrder(visibleProjects, sanitized);
      ...
      if (refreshed.success && refreshed.data) {
        update((state) => {
          ...
          const hiddenProjectIds = existing
            ? state.hiddenProjectIds.filter((id) => id !== existing.id)
            : state.hiddenProjectIds.filter((id) =>
                refreshed.data!.some((project) => project.id === id),
              );
          const visibleProjects = getVisibleProjects(
            refreshed.data!,
            hiddenProjectIds,
