...
object PsiNavigationHelper {
    ...
    fun navigateToLocation(project: Project, location: String): Boolean {
        ...
        // 修复：如果 methodName 实际上是文件后缀，则直接作为类跳转，不搜索方法/字段
        // 这解决了点击 "MyClass.java" 被识别为 MyClass 类找 java 方法，导致跳转到 Names.java 字段的问题
        ...
        ApplicationManager.getApplication().runReadAction {
            ...
            // === 策略核心：方法优先 (Method-First Strategy) ===
            
            // 1. 全网搜索方法 (Method Search)
            ...
            if (allMethods.isNotEmpty()) {
                ...
                val matchedMethods = methodsToCheck.filter { 
                    ...
                    // 策略升级：
                    // 1. 原始类名匹配
                    // 2. 首字母大写类名匹配 (处理 abc.method -> Abc.method)
                ...
                if (matchedMethods.isNotEmpty()) {
                    ...
                    // 继承关系匹配 (Inheritance Match)
                    // 场景: ChildClass.parentMethod
                    ...
                    // 兜底策略：如果还是没找到，但我们确实找到了一些名为 methodName 的方法
                    // 且当前上下文是"类名.方法名"的形式 (说明用户意图很明确是找这个方法)
                    // 那么直接返回找到的第一个方法 (优先项目源码)
            ...
            // 2. 如果方法没找到，尝试全网搜索字段/属性 (Field/Property Search)
        ...
        return targetElement != null || fallbackClass != null
    }
}
