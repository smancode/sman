...
DESIGN=$(ls -t ~/.sman/gstack/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t ~/.sman/gstack/projects/$SLUG/*-design-*.md 2>/dev/null | head -1)
...
```bash
HANDOFF=$(ls -t ~/.sman/gstack/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null | head -1)
...
```
Write to `~/.sman/gstack/projects/$SLUG/$USER-$BRANCH-ceo-handoff-$DATETIME.md`:
```markdown
...
```bash
source <(${SMAN_PLUGINS}/gstack-skills/bin/gstack-slug 2>/dev/null) && mkdir -p ~/.sman/gstack/projects/$SLUG/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p ~/.sman/gstack/projects/$SLUG/ceo-plans/archive
# For each stale plan: mv ~/.sman/gstack/projects/$SLUG/ceo-plans/{old-plan}.md ~/.sman/gstack/projects/$SLUG/ceo-plans/archive/
```

Write to `~/.sman/gstack/projects/$SLUG/ceo-plans/{date}-{feature-slug}.md` using this format:
...
rm -f ~/.sman/gstack/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null || true
```
...
`~/.sman/gstack/` (user config directory, not project files). The skill preamble
already writes to `~/.sman/gstack/sessions/` and `~/.sman/gstack/analytics/` — this is
the same pattern. The review dashboard depends on this data. Skipping this
command breaks the review readiness dashboard in /ship.

```bash
${SMAN_PLUGINS}/gstack-skills/bin/gstack-review-log '{"skill":"plan-ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","scope_proposed":N,"scope_accepted":N,"scope_deferred":N,"commit":"COMMIT"}'
```
...
```bash
${SMAN_PLUGINS}/gstack-skills/bin/gstack-review-read
```
...
- **B)** Keep in `~/.sman/gstack/projects/` only (local, personal reference)
- **C)** Skip
