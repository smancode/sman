...
```bash
${SMAN_PLUGINS}/gstack-skills/bin/gstack-review-read 2>/dev/null
```
...
```bash
ls -t ~/.sman/gstack-dev/evals/*-e2e-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -20
```
...
```bash
ls -t ~/.sman/gstack-dev/evals/*-llm-judge-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -5
```
...
```bash
eval $(${SMAN_PLUGINS}/gstack-skills/bin/gstack-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
...
```bash
$B snapshot -i -a -o ".sman/gstack/deploy-reports/post-deploy.png"
```
...
```bash
mkdir -p .sman/gstack/deploy-reports
```
...
Save report to `.sman/gstack/deploy-reports/{date}-pr{number}-deploy.md`.
