...
**用户分布预期：**
- 休闲用户（~30% 解锁率，约119分）→ 铂金
- 日常用户（~60% 解锁率，约415分）→ 钻石~星耀
- 重度用户（~90% 解锁率，约989分）→ 史诗

### 3.5 积分与等级的不可逆原则

**积分只升不降，等级只升不降。一旦发布，阈值永久生效。**

| 规则 | 说明 |
|------|------|
| 只能增加更多级别 | 比如在永恒之上加"至尊"等级，老用户不受影响 |
| 只能降低升级难度 | 比如把钻石门槛从200降到150，更多用户受益 |
| 绝不能提高门槛 | 比如把黄金从50改到80，会导致老用户掉级，**绝对禁止** |
| 绝不能减少级别 | 比如删掉"星耀"，会导致该级别用户无处安放，**绝对禁止** |
| 绝不能降低成就分值 | 比如把青铜从1分改到0.5分，等于变相提高门槛，**绝对禁止** |

**代码保障：** `AchievementEngine` 的 `calculateLevel()` 函数必须使用阈值下界做 `>=` 判断（而非范围判断），确保新增级别不会影响已有用户。
...
**成就 Tab：**
- 顶部：等级徽章（图形化）+ 总进度 + 积分 + "距离下一等级"进度条
- 分类筛选：全部 / 对话 / 高级功能 / 探索 / 协作 / Bot / 彩蛋
- 卡片网格（3 列）

**排行榜 Tab：**
- 维度 Tab：总榜 / 数量 / 对话 / 功能 / 协作 / Bot / 周榜
- 列表展示，自己高亮
- "再获得 X 分即可超越 @XXX" 追赶提示
- 离线提示：未连接星域时显示"仅显示本地数据"

### 5.3 等级徽章系统

**核心原则：少数图标 + 换色 = 10 个等级，低成本高辨识度。**

使用 Lucide 图标库的已有图标，通过颜色区分等级：
...
**徽章位置：**
- 页面顶部标题旁，始终可见（非 hover 弹出）
- 等级 hover 弹出框保留，展示完整进阶列表（含积分阈值）
- 聊天界面侧边栏头像旁显示迷你徽章（16px）

**等级颜色（与 Sman 暖灰底色和谐）：**

| 等级 | 浅色模式 | 深色模式 |
|------|---------|---------|
| 青铜 | `hsl(30, 45%, 38%)` | `hsl(30, 35%, 58%)` |
| 白银 | `hsl(210, 10%, 55%)` | `hsl(210, 8%, 68%)` |
| 黄金 | `hsl(42, 78%, 42%)` | `hsl(42, 72%, 55%)` |
| 铂金 | `hsl(210, 15%, 62%)` | `hsl(210, 12%, 72%)` |
| 钻石 | `hsl(190, 50%, 38%)` | `hsl(190, 45%, 55%)` |
| 星耀 | `hsl(340, 50%, 48%)` | `hsl(340, 45%, 58%)` |
| 王者 | `hsl(8, 60%, 45%)` | `hsl(8, 55%, 55%)` |
| 传说 | `hsl(260, 40%, 48%)` | `hsl(260, 35%, 58%)` |
| 史诗 | `hsl(25, 70%, 45%)` | `hsl(25, 65%, 55%)` |
| 永恒 | `hsl(160, 55%, 35%)` | `hsl(160, 50%, 45%)` |

### 5.4 卡片样式

- **已解锁**：完整颜色 + 等级图标 + tier 边框光效 + 解锁日期
- **进度中（未解锁）**：图标可见（非灰度）+ 进度条 + 当前值/目标值 + **解锁条件文字**（如"再对话 3 次即可解锁"）
- **未开始**：图标灰度 + 解锁条件文字 + "0/阈值"
- **彩蛋（hidden）**：锁定图标 + "???"，解锁后翻转揭示

**关键改进（来自用户评估）：锁定卡片必须显示解锁条件，给用户明确目标感。**

### 5.5 进度与动画

- **等级升级进度条**：`铂金 268 → 星耀 380，还差 112 分`，始终可见
- **进度条动画**：填充时 0.5s ease-out 平滑动画
- **积分跳动**：解锁成就时积分数字 +X 飘字动画
- **等级升级庆祝**：升级时全屏粒子特效（可选关闭），段位图标从旧变新
...
- 一键生成包含等级徽章 + 精选成就 + 统计数据的精美卡片图
- 类似 Spotify Wrapped / GitHub Profile Card
- 内置复制/分享按钮
