...
describe("tasksStore event flow", () => {
    beforeEach(() => {
        vi.resetModules();
        Object.keys(eventCallbacks).forEach(
            (key) => delete eventCallbacks[key],
        );
    ...
    it("接收 task-status 后创建活跃任务", async () => {
        const { tasksStore } = await import("../stores/tasks");
    ...
    it("接收编排事件后更新子任务、进度和验收结果", async () => {
        ...
        expect(currentState.orchestrationProgress.get("task-2").percent).toBe(
            1,
        );
        expect(currentState.tasks[0].output).toContain(
            "Acceptance for request",
        );
