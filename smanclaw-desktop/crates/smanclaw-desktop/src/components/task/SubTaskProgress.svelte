<script lang="ts">
  import type { SubTaskInfo, SubTaskStatus, OrchestrationProgress } from '../../lib/types';
...
<div class="subtask-progress">
  {#if progress}
    <div class="overall-progress">
      ...
      <div class="progress-percent">{Math.round(progress.percent * 100)}%</div>
    </div>
  {/if}
  ...
  {#if subtasks.length > 0}
    <div class="subtasks-list">
      ...
      {#each subtasks as subtask (subtask.id)}
        <div class="subtask-item" class:running={subtask.status === 'running'} class:completed={subtask.status === 'completed'} class:failed={subtask.status === 'failed'}>
          <div class="subtask-status" style="color: {getStatusColor(subtask.status)}">
          ...
          <div class="subtask-content">
            ...
            {#if subtask.depends_on.length > 0}
              <span class="subtask-deps">depends on: {subtask.depends_on.join(', ')}</span>
            {/if}
          </div>
          <span class="subtask-status-label" style="color: {getStatusColor(subtask.status)}">
